...
for i = 2:length(DR)
    ...
    L_k_new = L_k - Xk_new(3);
    Lambda_k_new = Lambda_k - Xk_new(4);
    V_Nk_new = V_Nk - Xk_new(1);
    V_Ek_new = V_Ek - Xk_new(2);

    % store results at each epoch into motion file
    result_single = [0.5*(i-1), L_k_new * rad_to_deg, Lambda_k_new * rad_to_deg, V_Nk_new, V_Ek_new, Heading(i)];
    Motion_5 = [Motion_5; result_single];
