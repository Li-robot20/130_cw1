function [latitude, longitude, height, v_north, v_east, v_down, outlier_res] = solver_no_outlier(time, Pseudoranges, Pseudorangesrates, omega_ie, c, outlier)
r_ea = [0;0;0];
v_ea = [0;0;0];
...
while distance>0.1
    ...
    for i = 1:length(index)
        ...
        if ismember(time, outlier(:,1)) & ismember(i, outlier(:,2))
            z_j = 0;
        else
            z_j = r_e(i) - ra(i) - x(4);
        end
...
for i=1:8
    if abs(v(i))>sqrt(Cv(i,i))*6
        ...
        outlier_res = [outlier_res; outlier_temp];
    end
end
